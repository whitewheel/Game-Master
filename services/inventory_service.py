...
def calc_carry(guild_id: int, owner: str):
    ...
    rows = fetchall(
        guild_id,
        "SELECT qty, metadata FROM inventory WHERE lower(owner)=lower(%s)",
        (owner,)
    )
    for r in rows:
        try:
    ...
    # --- Equipment items ---
    row = fetchone(guild_id, "SELECT equipment FROM characters WHERE name=%s", (owner,))
    ...
    execute(
        guild_id,
        "UPDATE characters SET carry_used=%s, updated_at=CURRENT_TIMESTAMP WHERE name=%s",
        (total_weight, owner)
    )
    return total_weight
...
def add_item(guild_id: int, owner, item_name, qty=1, metadata=None, user_id="0"):
    ...
    char = fetchone(guild_id, "SELECT carry_capacity, carry_used FROM characters WHERE name=%s", (owner,))
    ...
    row = fetchone(
        guild_id,
        "SELECT * FROM inventory WHERE lower(owner)=lower(%s) AND lower(item)=lower(%s)",
        (owner, item_name)
    )
    if row:
        new_qty = (row["qty"] or 0) + qty
        execute(guild_id, "UPDATE inventory SET qty=%s, updated_at=CURRENT_TIMESTAMP WHERE id=%s",
                (new_qty, row["id"]))
    else:
        # simpan owner apa adanya, item sudah dinormalkan
        execute(guild_id, "INSERT INTO inventory (owner, item, qty, metadata) VALUES (%s,%s,%s,%s)",
...
def remove_item(guild_id: int, owner, item_name, qty=1, user_id="0"):
    ...
    if new_qty > 0:
        ...
        execute(guild_id, "DELETE FROM inventory WHERE id=%s", (row["id"],))
...
def get_inventory(guild_id: int, owner):
    ...
    rows = fetchall(
        guild_id,
        "SELECT item, qty, metadata FROM inventory WHERE lower(owner)=lower(%s)",
        (owner,)
    )
...
def update_metadata(guild_id: int, owner, item_name, metadata: dict, user_id="0"):
    ...
    execute(guild_id, "UPDATE inventory SET metadata=%s, updated_at=CURRENT_TIMESTAMP WHERE id=%s",
