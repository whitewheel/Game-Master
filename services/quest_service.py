...
def add_quest(guild_id: int, title, detail="", rewards=None, hidden=False, user_id=0):
    ...
    execute(
        guild_id,
        """INSERT INTO quests
           (name, description, status, assigned_to, rewards, favor, tags, archived, rewards_visible)
...
def get_quest(guild_id: int, title):
    return fetchone(guild_id, "SELECT * FROM quests WHERE name=%s", (title,))

def list_quests(guild_id: int, status="all", include_archived=False):
    rows = fetchall(guild_id, "SELECT * FROM quests WHERE guild_id=%s", (guild_id,))
    if not rows:
...
def update_status(guild_id: int, title, new_status, archive=False):
    if archive:
        execute(guild_id, "UPDATE quests SET status=%s, archived=1, updated_at=CURRENT_TIMESTAMP WHERE name=%s",
                (new_status, title))
    else:
        execute(guild_id, "UPDATE quests SET status=%s, updated_at=CURRENT_TIMESTAMP WHERE name=%s",
                (new_status, title))
    return True

def set_rewards(guild_id: int, title, rewards: dict):
    execute(guild_id, "UPDATE quests SET rewards=%s, updated_at=CURRENT_TIMESTAMP WHERE name=%s",
            (json.dumps(rewards), title))
    return True

def set_rewards_visible(guild_id: int, title, visible: bool):
    execute(guild_id, "UPDATE quests SET rewards_visible=%s, updated_at=CURRENT_TIMESTAMP WHERE name=%s",
            (1 if visible else 0, title))
    return True

def assign_characters(guild_id: int, title, chars: list):
    execute(guild_id, "UPDATE quests SET assigned_to=%s, updated_at=CURRENT_TIMESTAMP WHERE name=%s",
...
async def complete_quest(guild_id: int, title, targets: list = None, user_id=0):
    ...
    if "gold" in rewards and rewards["gold"] > 0:
        for ch in targets:
            old = fetchone(guild_id, "SELECT gold FROM characters WHERE name=%s", (ch,))
            if old:
...
def archive_quest(guild_id: int, title):
    execute(guild_id, "UPDATE quests SET archived=1, updated_at=CURRENT_TIMESTAMP WHERE name=%s", (title,))
    return True
