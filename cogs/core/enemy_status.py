...
class EnemyStatus(commands.Cog):
    ...
    async def enemy_add(self, ctx, name: str, hp: int, energy: int, stamina: int):
        ...
        exists = fetchone(guild_id, "SELECT id FROM enemies WHERE name=%s", (name,))
        if exists:
            execute(guild_id, """
                UPDATE enemies
    ...
    async def enemy_show(self, ctx, *, name: str = None):
        guild_id = ctx.guild.id
        rows = [fetchone(guild_id, "SELECT * FROM enemies WHERE name=%s", (name,))] if name else fetchall(guild_id, "SELECT * FROM enemies")
        if not rows:
    ...
    async def enemy_remove(self, ctx, *, name: str):
        guild_id = ctx.guild.id
        row = fetchone(guild_id, "SELECT id FROM enemies WHERE name=%s", (name,))
        if not row:
            return await ctx.send(f"❌ Enemy **{name}** tidak ditemukan.")
        execute(guild_id, "DELETE FROM enemies WHERE name=%s", (name,))
