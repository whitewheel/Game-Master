...
def _ensure_char(guild_id: int, char_name: str):
    row = fetchone(guild_id, "SELECT * FROM characters WHERE name=%s", (char_name,))
    if not row:
...
def _save_companions(guild_id: int, char_name: str, comps: list):
    execute(
        guild_id,
        "UPDATE characters SET companions=%s, updated_at=CURRENT_TIMESTAMP WHERE name=%s",
...
def _format_effect(guild_id: int, eff: dict):
    ...
    name = eff.get("name") or eff.get("id") or "?%s?"
