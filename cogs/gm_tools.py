...
class GMTools(commands.Cog):
    ...
    async def inv_wipe(self, ctx, char: str):
        ...
        execute(guild_id, "DELETE FROM inventory WHERE LOWER(owner)=LOWER(%s)", (char,))
    ...
    async def equip_clear(self, ctx, char: str):
        ...
        execute(
            guild_id,
            "UPDATE characters SET equipment=%s, updated_at=CURRENT_TIMESTAMP WHERE LOWER(name)=LOWER(%s)",
            (json.dumps(eq), char)
        )
