...
def _migrate_hollow_visitors(guild_id: int):
    cols = ["name", "rarity", "chance", "description"]  # fixed columns, schema di Supabase
...
def _get_node(guild_id, name):
    ...
    return fetchone(guild_id, "SELECT * FROM hollow_nodes WHERE LOWER(name)=%s", (name.lower(),))
...
def _add_node(guild_id, name, zone, node_type):
    ...
    execute(guild_id,
            "INSERT INTO hollow_nodes (name, zone, type) VALUES (%s, %s, %s)",
            (name, zone, node_type))
...
def _edit_node(guild_id, name, entry):
    ...
    execute(guild_id, f"UPDATE hollow_nodes SET {set_clause}, updated_at=CURRENT_TIMESTAMP WHERE LOWER(name)=%s", params)
...
def _remove_node(guild_id, node_name):
    ...
    execute(guild_id, "DELETE FROM hollow_nodes WHERE LOWER(name)=%s", (node_name.lower(),))
...
def _reset_node(guild_id, node_name):
    ...
    execute(
        guild_id,
        "UPDATE hollow_nodes SET vendors_today='[]', visitors_today='[]', event_today='{}', updated_at=CURRENT_TIMESTAMP WHERE LOWER(name)=%s",
        (node_name.lower(),)
    )
    return f"♻️ Node `{node_name}` di-reset."

def _get_logs(guild_id, node_name, n=5):
    return fetchall(guild_id, "SELECT * FROM hollow_log WHERE node=%s ORDER BY id DESC LIMIT ?", (node_name, n))
...
def _add_npc(guild_id, node_name, npc_name, chance=50, rarity="common"):
    ...
    execute(guild_id, "UPDATE hollow_nodes SET npcs=%s WHERE name=%s", (_json_dump(npcs), node_name))
    ...
    for n in npcs:
        if isinstance(n, str):
            ...
            if n.get("name", "").lower() != npc_name.lower():
                norm.append(n)
    execute(guild_id, "UPDATE hollow_nodes SET npcs=%s WHERE name=%s", (_json_dump(norm), node_name))
    return f"🗑️ Vendor `{npc_name}` dihapus dari `{node_name}`."

# ---------- Visitors (global) ----------
def _add_visitor(guild_id, name):
    execute(guild_id, "INSERT INTO hollow_visitors (guild_id, name) VALUES (%s, %s) ON CONFLICT DO NOTHING", (name,))
    return f"✅ Visitor `{name}` ditambahkan."

def _remove_visitor(guild_id, name):
    execute(guild_id, "DELETE FROM hollow_visitors WHERE LOWER(name)=%s", (name.lower(),))
    return f"🗑️ Visitor `{name}` dihapus."

def _edit_visitor(guild_id, name, entry):
    row = fetchone(guild_id, "SELECT * FROM hollow_visitors WHERE LOWER(name)=%s", (name.lower(),))
    if not row:
    ...
    params = list(updates.values()) + [name.lower()]
    execute(guild_id, f"UPDATE hollow_visitors SET {set_clause} WHERE LOWER(name)=%s", params)
...
def _add_event(guild_id, name):
    execute(guild_id, "INSERT INTO hollow_events (guild_id, name) VALUES (%s, %s) ON CONFLICT DO NOTHING", (name,))
    return f"✅ Event `{name}` ditambahkan."

def _remove_event(guild_id, name):
    execute(guild_id, "DELETE FROM hollow_events WHERE LOWER(name)=%s", (name.lower(),))
...
def _edit_event(guild_id, name, entry):
    ...
    execute(guild_id, f"UPDATE hollow_events SET {set_clause} WHERE name=%s", params)
...
def _assign_event(guild_id, event_name, node_name):
    ev = fetchone(guild_id, "SELECT * FROM hollow_events WHERE LOWER(name)=%s", (event_name.lower(),))
    if not ev:
        return f"❌ Event `{event_name}` tidak ditemukan."
    execute(guild_id, "UPDATE hollow_nodes SET event_today=%s WHERE LOWER(name)=%s",
...
def _clear_event(guild_id, node_name):
    execute(guild_id, "UPDATE hollow_nodes SET event_today='{}' WHERE LOWER(name)=%s", (node_name.lower(),))
...
def _add_trait(guild_id, node_name, trait):
    ...
    if trait not in traits:
        traits.append(trait)
    execute(guild_id, "UPDATE hollow_nodes SET traits=%s WHERE name=%s", (_json_dump(traits), node_name))
...
def _add_type(guild_id, node_name, t):
    ...
    if t not in types:
        types.append(t)
    execute(guild_id, "UPDATE hollow_nodes SET types=%s WHERE name=%s", (_json_dump(types), node_name))
...
def _roll_daily(guild_id, node_name, time_label="day"):
    ...
    execute(
        guild_id,
        "UPDATE hollow_nodes SET vendors_today=%s, visitors_today=%s, event_today=%s, updated_at=CURRENT_TIMESTAMP WHERE name=%s",
        (_json_dump(vendors_today), _json_dump(visitors_today), json.dumps(event_today, ensure_ascii=False), node_name)
    )
    execute(
        guild_id,
        "INSERT INTO hollow_log (node, vendors, visitors, event, time) VALUES (%s, %s, %s, %s, %s)",
...
class Hollow(commands.Cog):
    ...
    async def editvisitor(self, ctx, *, entry: str):
        try:
            ...
            parts = re.split(r'\s+(%s=\w+=)', entry, 1)
