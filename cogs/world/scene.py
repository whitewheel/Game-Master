...
class Scene(commands.Cog):
    ...
    async def scene_create(self, ctx, *, entry: str):
        ...
        execute(
            guild_id,
            "INSERT INTO scenes (guild_id, name, description) VALUES (%s, %s, %s) ON CONFLICT (guild_id, name) DO UPDATE SET description=EXCLUDED.description",
            (name, desc),
        )
    ...
    async def scene_edit(self, ctx, *, entry: str):
        ...
        row = fetchone(guild_id, "SELECT * FROM scenes WHERE name=%s", (name,))
        if not row:
            return await ctx.send(f"❌ Scene **{name}** tidak ditemukan.")

        execute(
            guild_id,
            "UPDATE scenes SET desc=%s, factions=%s, danger=%s, updated_at=CURRENT_TIMESTAMP WHERE name=%s",
