...
class Crafting(commands.Cog):
    ...
    async def bp_detail(self, ctx: commands.Context, *, name: str):
        ...
        ensure_tables(ctx.guild.id)
        bp = fetchone(ctx.guild.id, "SELECT * FROM blueprints WHERE name=%s", (name,))
        if not bp:
    ...
    async def bp_learn(self, ctx: commands.Context, player: str, *, blueprint: str):
        ...
        ensure_tables(ctx.guild.id)
        bp = fetchone(ctx.guild.id, "SELECT name FROM blueprints WHERE name=%s", (blueprint,))
        if not bp:
            return await ctx.send("❌ Blueprint tidak ditemukan di database.")

        execute(
            ctx.guild.id,
            "INSERT INTO known_blueprints (guild_id, player, blueprint) VALUES (%s,%s,%s) ON CONFLICT DO NOTHING",
            (player, bp["name"]),
        )
        embed = discord.Embed(
    ...
    async def bp_known(self, ctx: commands.Context, player: str):
        ...
        rows = fetchall(
            ...
            "JOIN blueprints b ON b.name = k.blueprint WHERE k.player=%s ORDER BY b.name ASC",
            (player,),
        )
        if not rows:
    ...
    async def craft_start(self, ctx: commands.Context, player: str, *, blueprint: str):
        ...
        known = fetchone(
            guild_id,
            "SELECT 1 FROM known_blueprints WHERE player=%s AND blueprint=%s",
            (player, blueprint),
        )
        if not known:
            embed = discord.Embed(
        ...
        # Ambil blueprint
        bp = fetchone(guild_id, "SELECT * FROM blueprints WHERE name=%s", (blueprint,))
        if not bp:
            return await ctx.send("❌ Blueprint tidak ditemukan.")
        ...
        try:
            row_stat = fetchone(
                guild_id,
                "SELECT crafting_lvl FROM characters WHERE name=%s",
                (player,),
            )
        ...
        execute(
            guild_id,
            "INSERT INTO crafting (guild_id, player, blueprint, progress) VALUES (%s,%s,%s,0) ON CONFLICT (guild_id, player) DO UPDATE SET blueprint=EXCLUDED.blueprint, progress=0",
            (player, bp["name"]),
        )
    ...
    async def craft_progress(self, ctx: commands.Context, player: str, value: int):
        ...
        row = fetchone(guild_id, "SELECT * FROM crafting WHERE player=%s", (player,))
        if not row:
            return await ctx.send("❌ Tidak ada crafting aktif untuk player ini.")

        bp = fetchone(guild_id, "SELECT * FROM blueprints WHERE name=%s", (row["blueprint"],))
        if not bp:
            return await ctx.send("❌ Blueprint tidak ditemukan.")

        old = int(row["progress"])
        new = max(0, old + int(value))
        execute(guild_id, "UPDATE crafting SET progress=%s WHERE player=%s", (new, player))
    ...
    async def craft_finish(self, ctx: commands.Context, player: str):
        ...
        if not row:
            return await ctx.send("❌ Tidak ada crafting aktif.")
        bp = fetchone(guild_id, "SELECT result, target_progress FROM blueprints WHERE name=%s", (row["blueprint"],))
        if not bp:
            return await ctx.send("❌ Blueprint tidak ditemukan.")
        ...
        execute(guild_id, "DELETE FROM crafting WHERE player=%s", (player,))
    ...
    async def craft_cancel(self, ctx: commands.Context, player: str):
        ...
        ensure_tables(ctx.guild.id)
        execute(ctx.guild.id, "DELETE FROM crafting WHERE player=%s", (player,))
        embed = discord.Embed(
    ...
    async def _auto_finish(self, ctx: commands.Context, player: str, blueprint_name: str):
        ...
        guild_id = ctx.guild.id
        bp = fetchone(guild_id, "SELECT result FROM blueprints WHERE name=%s", (blueprint_name,))
        if not bp:
